...
void mlfq_push(struct proc *p,int present_queue_number) {
  if(queue[present_queue_number].tail >= NPROC) {
    // printf("tail %d\n",queue[present_queue_number].tail);
    // printf("NPROC %d\n",NPROC);
    // printf("Entered\n");
...
int fork(void)
{
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  // for(int i=0;i<32;i++) {
  //   printf("In proc.c before loop %d %d\n",i,np->sysCounter[i]);
  // }
  // np->syscall_maks = p->syscall_maks;
  // // acquire(&p->lock);
  // for (int i = 0; i < 32; i++) {
  //   np->sysCounter[i] = p->sysCounter[i];  // Copy parent's syscall counters to the child
  // }

  // for(int i=0;i<32;i++) {
  //   printf("In proc.c after loop %d %d\n",i,np->sysCounter[i]);
  // }
  // release(&p->lock);
  return pid;
}
  ...
  for (;;)
  {
    ...
    for(p = proc;p < &proc[NPROC];p++) {
      if(p->state == RUNNABLE && p->status_queue == 0) {
        int present_queue_number;
        present_queue_number = p->priority_queue_number;
        // mlfq_push(p,present_queue_number);
        ...
        p->number_process = queue[present_queue_number].tail - 1; 
        p->status_queue = 1;
        // queue[present_queue_number].tail = stored_tail_value;
      }
      if(p->state == UNUSED) {
        continue;
      }
    ...
    struct proc *current_process_track = 0;
    for(int i=0;i<queue_size;i++) {
      ...
      while(!(queue[i].tail <= 0)) {
        ...
        if(current_process_track->state == RUNNABLE) {
          // mlfq_push_front(current_process_track,current_process_track->priority_queue_number);
          ...
          // current_process_track->priority_queue_number = current_process_track->priority_queue_number;
